...
def initialize_services(config: dict) -> tuple[SessionService, AnalysisService | None, WebSocketManager, dict[str, str]]:
    security=load_yaml(ROOT/"configs/security.yaml");privacy=load_privacy_config(ROOT/"configs/privacy.yaml");assert_privacy_defaults(privacy);digest=config_hash([ROOT/"configs/api.yaml",ROOT/"configs/realtime.yaml",ROOT/"configs/risk_engine.yaml",ROOT/"configs/security.yaml",ROOT/"configs/privacy.yaml"]);config["config_hash"]=digest;config["model_versions"]={"phase3_model_version":"unavailable-until-v4-trained","phase4_model_version":"speechbrain-ecapa","risk_engine_version":"phase5-v1","preprocessing_version":"phase2-v1"};audit=AuditLog(ROOT/security["audit"]["path"],digest) if security["audit"]["enabled"] else None
