...
class AnalysisService:
    ...
    async def analyze(self, session_id: str, audio: np.ndarray) -> dict:
        ...
        try:
            ...
            result = {"session_id":session_id,"segment_id": segment_id,"timestamp":__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(),"synthetic_probability": raw.get("synthetic_probability"), "speaker_similarity": raw.get("speaker_similarity"), "risk_score": combined["risk_score"], "risk_level": combined["risk_level"], "decision": combined["decision"], "recommended_action": combined["recommended_action"],"engine_version":risk.get("engine_version"),"config_version":risk.get("configuration_version"),"config_hash":self.config.get("config_hash"),"model_versions":self.config.get("model_versions",{})}
