...
def main() -> None:
    ...
    info = json.loads((model_dir / "model_config.json").read_text()); model_name = info["base_model"]; device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    csv = resolve_path(cfg["paths"]["test_csv"], base); maximum = cfg["training"].get("max_test_samples") if cfg["training"].get("debug_mode") else None
    ...
    predictions = pd.DataFrame(predict_loader(model, loader, device)); report = resolve_path(cfg["paths"]["report_dir"], base); metrics = save_evaluation_reports(predictions, report, float(cfg["evaluation"]["threshold"]), "test")
    ...
    if cfg["training"].get("debug_mode"): print("THIS IS NOT A VALID MODEL PERFORMANCE RESULT (debug/dummy data).")
