...
def main()->None:
    ...
    genuine=int((validation.trial_type=="genuine").sum());impostor=int((validation.trial_type=="impostor").sum());demo=bool(frame.get("demo",pd.Series(dtype=bool)).fillna(False).any());minimum=int(cfg["verification"].get("minimum_calibration_trials_per_class",20))
    calibrated=resolve_path(cfg["paths"]["calibrated_threshold"],base);calibrated.parent.mkdir(parents=True,exist_ok=True);calibrated.write_text(json.dumps({"threshold":threshold,"selected_on":"validation","criterion":"eer","genuine_validation_trials":genuine,"impostor_validation_trials":impostor,"demo":demo,"usable_for_decision":not demo and genuine>=minimum and impostor>=minimum},indent=2))
