...
def main() -> None:
    ...
    limits = {name: tc.get(f"max_{name}_samples") if tc.get("debug_mode") else None for name in frames}
    model_name = cfg["model"].get("debug_name") if tc.get("debug_mode") else cfg["model"]["name"]
    ...
    if tc.get("debug_mode"): logging.warning("DEBUG MODE / DUMMY DATA: THIS IS NOT A VALID MODEL PERFORMANCE RESULT.")
