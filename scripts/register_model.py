...
def main():
 ...
 if a.type=="synthetic_detector":info=load_json(target/"model_config.json",{});version=a.version or f"v{info.get('training_epochs_completed','unknown')}-debug";metadata={"dataset_version":dataset_metadata(ROOT/"data/metadata/train.csv")["dataset_version"],"validation_metrics":info.get("validation_metrics"),"test_metrics":info.get("test_metrics"),"threshold":info.get("threshold"),"config_hash":sha256_file(ROOT/"configs/training.yaml")};name=info.get("configured_base_model","wav2vec2")
